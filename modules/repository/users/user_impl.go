...
func (repo *UserRepoImpl) RepoList(ctx context.Context, limit, offset uint64) (resUser []modelsUser.User, err error) {
	...
	sqlList := "SELECT id,name,email,created_at,updated_at FROM users Limit $1 offset $2"
...
func (repo *UserRepoImpl) RepoFindByid(ctx context.Context, id uint64) (resUser modelsUser.User, err error) {
	...
	sqlFind := "SELECT id,name,email,created_at,updated_at FROM users WHERE id=$1"
	...
	if row.Next() {
		err = row.Scan(&resUser.Id, &resUser.Name, &resUser.Email, &resUser.Created_At, &resUser.Updated_At)
		if err != nil {
			return
		}
	} else {
...
func (repo *UserRepoImpl) RepoCreate(ctx context.Context, userIn modelsUser.User) (resUser modelsUser.User, err error) {
	...
	sqlCreate := "INSERT INTO users(name,email,username,password,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6)"
	_, err = repo.DB.ExecContext(ctx, sqlCreate, userIn.Name, userIn.Email, userIn.Username, userIn.Password, userIn.Created_At, userIn.Updated_At)
	if err != nil {
		return
	}
...
func (repo *UserRepoImpl) RepoUpdate(ctx context.Context, userIn modelsUser.User) (resUser modelsUser.User, err error) {
	...
	sqlUpdate := "UPDATE users SET name=$1,email=$2,updated_at=$3 WHERE id=$4"
	_, err = repo.DB.ExecContext(ctx, sqlUpdate, userIn.Name, userIn.Email, userIn.Updated_At, userIn.Id)
	if err != nil {
		return
	}
	resUser.Name = userIn.Name
